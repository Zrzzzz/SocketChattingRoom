...
class GuiManager():
    ...
    def GetFrame(self, type, client, user=None):
        frame = self.frameDict.get(type)
 
        if frame is None:
            frame = self.CreateFrame(type, client, user)
            self.frameDict[type] = frame
 
        return frame
 
    def CreateFrame(self, type, client, user):
        if type == 0:
            return ClientLoginView(parent=None, id=type, updater=self.UpdateUI, client=client)
        elif type == 1:
            return ClientMainView(parent=None, id=type, updater=self.UpdateUI, client=client, user=user)